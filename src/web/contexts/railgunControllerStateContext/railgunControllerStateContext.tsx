...
import sepoliaCheckpoint from './sepolia-checkpoint.json'
...
const RailgunControllerStateProvider: React.FC<any> = ({ children }) => {
  ...
  const loadPrivateAccount = useCallback(async () => {
    ...
    try {
      ...
      for (const item of tracked) {
        console.log('[RailgunContext - LPA] loading account', item)
        ...
        console.log('[RailgunContext - LPA] account loaded')
        const zkAddress = await account.getRailgunAddress()
        console.log('[RailgunContext - LPA] get account from cache')
        ...
        if (!cached || !cached.lastSyncedBlock) {
          ...
          await account.syncWithLogs(sepoliaCheckpoint.logs as unknown as Log[], true)
        ...
        const provider = getInfuraProvider(chainId, memoizedBgState.infuraApiKey)
  ...
  }, [selectedAccount, memoizedBgState.chainId, railgunAccountsState.status, getDerivedKeysFromBg, getAccountCacheFromBg, fireBg, memoizedBgState])
  ...
  }, [selectedAccount, memoizedBgState.chainId, loadPrivateAccount])

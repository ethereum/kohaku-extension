...
const DepositsPositions: FC<Props> = ({
  openTab,
  sessionId,
  setOpenTab,
  initTab,
  ...
  console.log({ accountsOps })
  useEffect(() => {
    // Optimization: Don't apply filtration if we are not on Deposits tab
    if (!account?.addr || openTab !== 'deposits') return
    ...
    ({ item }: any) => {
      ...
      if (item === 'empty') {
        return (
          ...
          >
            {t('No deposit transactions for {{account}}', {
      ...
      if (!initTab?.deposits || !item || item === 'keep-this-to-avoid-key-warning') return null
  ...
  // Filter transactions to only show deposits (privateDepositRequest)
  const filteredItems =
    accountsOps?.[sessionId]?.result.items.filter((item: any) => {
      // Check if this is a deposit transaction to privacy pools
      // This assumes the transaction has some identifier for deposit type
      return item.type === 'privateDepositRequest' || item.meta?.type === 'privateDepositRequest'
    }) || []
  ...
  return (
    <DashboardPageScrollContainer
      ...
      data={[
        ...
        ...(initTab?.deposits && filteredItems.length ? filteredItems : []),
...
export default React.memo(DepositsPositions)

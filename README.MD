...
1. **Clone the Kohaku SDK**: Clone the repository from [Kohaku SDK](https://github.com/ethereum/kohaku).

2. **Install and build**: From the Kohaku SDK repository root, run `pnpm install` and then `pnpm build`. After that, navigate out of the Kohaku SDK repository.

3. **Clone this repository**: Clone this repo and switch to the `demo/devconnect` branch.

4. **Initialize submodules**: Run `git submodule init` and then `git submodule update`.

5. **Verify local dependency**: Ensure the `@kohaku-eth/railgun` dependency in `package.json` points to your local filepath for the Kohaku SDK.

6. **Create and fill in .env file**: Create a `.env` file in this repo root. See `.env-sample` for helpful example. Fill it in with the following mandatory env variables:
   ...
   | **REACT_APP_PIMLICO_API_KEY**    | Required for ERC4337 txs. You can generate your API Key [here](https://dashboard.pimlico.io).           |
...
7. **Install dependencies**: Run `yarn install` to install all kohaku-extension dependencies.

8. **Build the extension**: Execute `yarn web:webkit` to build the kohaku-extension. Once you see the two “web compiled successfully” logs, you can stop the process (or keep it running if you prefer). Just open a new terminal tab for the next step.

9. **Load the unpacked extension**: Load the unpacked extension in Chrome (from the build folder), and you’re good to go!
